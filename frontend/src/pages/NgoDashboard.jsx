...
export default function NgoDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const foodResponse = await fetch(`${process.env.BACKEND_URL}/listings/`);
        const historyResponse = await fetch(
          `${process.env.BACKEND_URL}/ngo/listings/${user.id}`
        );
        ...
        await Promise.all(picked.map(async (item) => {
          const res = await fetch(`${process.env.BACKEND_URL}/reviews/${item._id}`);
  ...
  const sendMessage = async () => {
    ...
    try {
      await fetch("`${process.env.BACKEND_URL}`/chat/save", {
        method: "POST",
  ...
  const handleOpenChat = async (donor) => {
    ...
    try {
      const existingChat = await fetch(`${process.env.BACKEND_URL}/chat/${user.id}`);
      ...
      const historyRes = await fetch(`${process.env.BACKEND_URL}/chat/history/${roomId}`);
  ...
  const createNewRoom = async (ngo, user) => {
    const res = await fetch("`${process.env.BACKEND_URL}`/chat/create", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleClaim = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.BACKEND_URL}/listings/${selectedListing._id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        ...
        const updatedFoodResponse = await fetch("`${process.env.BACKEND_URL}`/listings/");
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const response = await fetch(`${process.env.BACKEND_URL}/listings/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        ...
        const foodRes = await fetch(`${process.env.BACKEND_URL}/ngo/listings/${user.id}`);
  ...
  const handleCancel = async (id) => {
    try {
      ...
      const historyRes = await fetch(`${process.env.BACKEND_URL}/ngo/listings/${user.id}`);
      const history = await historyRes.json();
