...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchListings = async () => {
      try {
        const response = await fetch(`${process.env.BACKEND_URL}/listings/user/${user.id}`);
    ...
    const fetchMessages = async () => {
      try {
        const response = await fetch(`${process.env.BACKEND_URL}/chat/user/` + user.id);
        const data = await response.json();
    ...
    const fetchReviews = async () => {
      try {
        const response = await fetch(`${process.env.BACKEND_URL}/reviews/user/${user.id}`);
        const data = await response.json();
    ...
    const fetchUsers = async () => {
      try {
        ...
        const userPromises = userIds.map(userId =>
          fetch(`${process.env.BACKEND_URL}/users/${userId}`).then(res => res.json())
        );
  ...
  const handleChatClick = async (chatId) => {
    ...
    try {
      const historyRes = await fetch(`${process.env.BACKEND_URL}/chat/history/${chatId}`);
  ...
  const handleModalSubmit = async (data) => {
    ...
    try {
      ...
      const response = await fetch("`${process.env.BACKEND_URL}`/listings/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
